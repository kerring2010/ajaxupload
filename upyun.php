...
class UpYun {
     ...
     */
    public function writeFile($path, $file, $auto_mkdir = False, $opts = NULL, $isFigureBed = false) {
        ...
        $this->_file_infos = $this->_do_request('PUT', $path, $opts, $file, $isFigureBed);
     ...
     * @return mixed
     */
    protected function _do_request($method, $path, $headers = NULL, $body = NULL, $file_handle = NULL, $isFigureBed = false) {
        $uri = "/{$this->_bucketname}{$path}";
        $ch = curl_init("http://{$this->endpoint}{$uri}");

        // 增加默认的旋转参数 by kerring
        $_headers = array('Expect:', 'x-gmkerl-rotate: auto');
        ...
        if ($http_code == 200) {
            ...
            if ($isFigureBed) {
                return $http_code . ":" . $header_string;
            }
    ...
    private function _getHeadersData($text) {
        ...
        foreach($headers as $header) {
            $header = trim($header);
            if (is_int(stripos($header, 'x-upyun'))) {
