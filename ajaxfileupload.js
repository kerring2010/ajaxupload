...
// 自适应图片居中 by kerring
// <img src='image.jpg' onload="javascript:DrawImage(this,80,80);" style="visibility:hidden;" />
...
//上传图片
// 必要元素prev-->img,next-->input
function ajaxFileUpload(el,url){
    var obj = $(el)
      , img = obj.prev().find('img')
      , input = obj.next().find('input')
      , file_id = "up_" + (new Date).getTime() + parseInt(1e5 * Math.random())
    ...
    $.ajaxFileUpload({
        ...
        dataType: 'json',
        success: function (d){
            console.log('ajaxFileUpload',d)
            if (typeof (d.code) != 'undefined' && d.code == 0) {
                // 更新图片
